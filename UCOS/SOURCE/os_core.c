...
static  void  OS_SchedEDF(void)
{
    ...
    OS_EXIT_CRITICAL();
    while (ptcb->OSTCBPrio < OS_TASK_IDLE_PRIO-2) {
        OS_ENTER_CRITICAL();
        if (ptcb->OSTCBDly == 0&&ptcb->OSTCBPrio > 1) {
            if (ptcb->deadline < min) {
                min = ptcb->deadline;
...
INT8U  OS_TCBInit (INT8U    prio,
                   ...
                   INT32U   stk_size,
                   void    *pext,
                   INT16U   opt, INT16U c, INT16U t,INT16U deadline)/*进行了改动添加了c t*/
{
    OS_TCB    *ptcb;
#if OS_CRITICAL_METHOD == 3u                               /* Allocate storage for CPU status register */
    OS_CPU_SR  cpu_sr = 0u;
