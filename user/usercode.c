...
//{
//    int i=0;
//	unsigned c = OSTCBCur->c;
//	unsigned T = OSTCBCur->t;
//	OSTimeDly(100);
//    printf("两个任务交替运行，E2task1,优先级%d\n",OSPrioCur);
//	OSTaskChangePrio(OSPrioCur, 2);
//	printf("优先级为%d的任务输出%d\n", OSPrioCur, i++);
//	for(;;)
//	{
//		unsigned n = T - OSTime %T;
//		printf("优先级为%d的任务输出%d\n",OSPrioCur,i++);
//		if (OSTCBCur->c == 0 && n) {
//			OSTaskChangePrio();
//			OSTimeDly(n);
...
void E2_task1(void *pParam)
{
	...
	for(;;)
	{
		...
		OS_ENTER_CRITICAL();
		OSTCBCur->n++;
		OSTCBCur->deadline = OSTCBCur->deadline + OSTCBCur->n *OSTCBCur->t;
		OSTCBCur->c = C;
		printf("%d  %d 任务即将阻塞%d个时间片\n", OSTCBCur->t, OSTime,(OSTCBCur->t - (OSTime-100) % OSTCBCur->t));
		OS_EXIT_CRITICAL();
		OSTimeDly(OSTCBCur->t - OSTime%OSTCBCur->t);
		printf("剩余执行时间%d\n", OSTCBCur->c);
	}
}
