...
int main(int argc, char **argv)
{
 ...
 //       return(1); 	
	...
	OSInit();
	printf("请输入两个任务的截止时间\n");
	OSTaskCreate(TaskStart, 0, &TaskStk[1][TASK_STK_SIZE-1], TaskStart_Prio,0,0,0);
	unsigned deadline;
	unsigned deadline1;
	unsigned c1;
	unsigned c2;
	unsigned t1;
	unsigned t2;
	scanf("%d %d", &deadline, &deadline1);
	printf("请输入两个任务每个周期所需的执行时间");
	scanf("%d %d", &c1, &c2);
	printf("请输入两个任务每个周期所需的周期");
	scanf("%d %d", &t1, &t2);
	unsigned prio = 5;
	OSTaskCreate(E2_task1, 0, &TaskStk[5][TASK_STK_SIZE - 1], prio, c1, t1, deadline);
	OSTaskCreate(E2_task2, 0, &TaskStk[6][TASK_STK_SIZE - 1], prio + 1, c2, t2, deadline1);
	//switch(Experiment) 
	//{
	//	case 1://一个任务运行
